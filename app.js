...
function updateConsumptionSummary(activePanelId) {
    ...
    const recentPanel = document.getElementById('recentRepostajesPanel');
    ...
    renderRecentRepostajes(selectedVehicleId);
...
function renderRecentRepostajes(vehicleId) {
    const container = document.getElementById('recentRepostajesList');
    container.innerHTML = '';

    const ultimos = getHistorialConConsumo(vehicleId).slice().reverse().slice(0, 2);
    ...
    ultimos.forEach(h => container.appendChild(createRepostajeRowElement(h)));
}
